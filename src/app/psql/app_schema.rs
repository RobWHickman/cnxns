...
WITH used_players AS (
    SELECT player1_id AS player_id FROM connections.public.daily_selection
    UNION
    SELECT player2_id AS player_id FROM connections.public.daily_selection
),
possible_players AS (
    SELECT p.player_id, p.full_name, COUNT(DISTINCT m.match_id) AS n
    FROM connections.public.players p 
    JOIN connections.public.player_stats ps ON ps.player_id = p.player_id
    JOIN connections.public.matches m ON ps.match_id = m.match_id
...
INSERT INTO connections.public.daily_selection (date, player1_id, player1_full_name, player2_id, player2_full_name)
SELECT CURRENT_DATE, player1_id, player1_full_name, player2_id, player2_full_name
FROM selected_players
WHERE NOT EXISTS (
    SELECT 1 FROM connections.public.daily_selection 
