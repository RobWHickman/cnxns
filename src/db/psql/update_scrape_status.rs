...
WITH scraped_matches AS (
    SELECT 
        ...
        m.match_id
    FROM pib_db.connections.matches m 
    JOIN pib_db.connections.player_stats ps
        ON ps.match_id = m.match_id 
    GROUP BY m.match_id
)
UPDATE pib_db.connections.matches 
SET data_count = 1,
    updated_at_utc = NOW()
FROM scraped_matches sm
WHERE pib_db.connections.matches.match_id = sm.match_id
  AND sm.original_count > 0 
  AND sm.is_integer_division = true;

UPDATE pib_db.connections.league_seasons ls
