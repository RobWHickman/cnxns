...
fn get_match_info(db_client: &mut PgClient) -> Result<Vec<MatchInfo>, Box<dyn std::error::Error>> {
    let rows: Vec<postgres::Row> = db_client.query(
        ...
        FROM matches 
        WHERE match_id NOT IN (SELECT DISTINCT match_id FROM player_stats)",
        &[],
    )?;
...
fn persist_player_data(
    ...
    db_client.execute(
        "INSERT INTO players (player_id, full_name, nationality, current_club, active) 
         VALUES ($1, $2, $3, $4, $5) 
...
fn persist_match_stats(
    ...
    for (variable, value) in stats {
        db_client.execute(
            "INSERT INTO player_stats (match_id, team_id, player_id, variable, value) 
             VALUES ($1, $2, $3, $4, $5) 
