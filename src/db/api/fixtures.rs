...
fn check_league_status(
    ...
    let query = "SELECT number_matches, data_count FROM pi_db.pi_db.connections.league_seasons WHERE league_id = $1 AND season_id = $2";
...
fn persist_league_fixtures(
    ...
    db_client.execute(
        "INSERT INTO pi_db.pi_db.connections.league_seasons (league_id, league_name, season_id, number_matches) 
    ...
    for match_data in matches {
        ...
        match db_client.execute(
            "INSERT INTO pi_db.pi_db.connections.matches (league_id, season_id, match_id, home_team_id, home_team_name, away_team_id, away_team_name, match_date, data_count)
